...
const parse = midiFile => {
  ...
  midi.getEvents().forEach(event => {
    ...
    else if (data && data.match(/^\[(section|prc)/)) {
      hasSections = true;
      sections.push({ time: event.playTime, section: data });
      ...
      if (event.param1 == 116) hasStarPower = true;
        ...
        if (diff && event.subtype == 9) {
          ...
          if (previous[diff]) {
            ...
            if (distance > 0 && distance < 5 && event.track == previous[diff].track) {
              brokenNotes.push({
          ...
          if (!previous[diff] || previous[diff].time != event.playTime) previous[diff] = { time: event.playTime, track: event.track };
  ...
  brokenNotes.forEach(note => {
    note.section = sections[sections.findIndex(section => section.time > note.time) - 1] || sections[sections.length - 1];
  });
...
module.exports = midiFile => parse(midiFile);
